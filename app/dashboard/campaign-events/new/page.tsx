...
export default function NewCampaignEventPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error: insertError } = await supabase.from("campaign_events").insert(record);
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-2">Add Campaign Event</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card space-y-6">
        ...
        <div>
          ...
          <input id="link_url" name="link_url" type="url" className="input" placeholder="Enter link url" />
        </div>
        <div>
        ...
        <div>
          ...
          <input id="event_data" name="event_data" type="text" className="input" placeholder="Enter event data" />
        </div>

        <div className="flex items-center gap-3 pt-4 border-t">
          <button type="submit" disabled={loading} className="btn-primary">
            {loading ? "Saving..." : "Create Campaign Event"}
          </button>
